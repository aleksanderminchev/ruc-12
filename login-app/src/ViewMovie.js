            ...
            <div className="container mt-5">
                <div className="row">
                    ...
                    <div className="col-md-8 text-light">
                        <h2 style={{ fontSize: '55px', color: 'yellow', fontFamily: 'Trajan Pro, serif', fontWeight: 'bold' }}>{movie.title}</h2>
                        <p style={{ fontSize: '24px', borderBottom: '1px solid white', paddingBottom: '10px' }}>{movie.description}</p>
                        <h4 style={{ fontSize: '40px', color: 'yellow', paddingBottom: '10px' }}>Directors:</h4>
                        <ul style={{ borderBottom: '1px solid white', paddingBottom: '10px' }}>
                            {movie.directors.map((director, index) => (
                                <li key={index} style={{ fontSize: '30px' }}>{director}</li>
                            ))}
                        </ul>
                        <div style={{ paddingBottom: '10px' }}>
                            {/* Empty div for the white line */}
                        </div>
                        <h4 style={{ fontSize: '35px', color: 'yellow',  paddingBottom: '10px' }}>Cast:</h4>
                        <div className="row">
                            <div className="col-md-6">
                                <ul>
                                    {castFirstHalf.map((actor, index) => (
                                        <li key={index} style={{ fontSize: '25px' }}>{actor}</li>
                                    ))}
                                </ul>
                            </div>
                            <div className="col-md-6">
